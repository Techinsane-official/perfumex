...
export class PlaywrightAmazonDEScraper extends PlaywrightBaseScraper {
  ...
  async searchProducts(searchTerm: string): Promise<ScrapedProductData[]> {
    try {
      ...
      console.log(`🔍 Playwright Amazon DE: Searching for "${searchTerm}"`);
      
      await this.navigateToUrl(searchUrl);
      console.log(`✅ Playwright Amazon DE: Navigation completed`);
      
      await this.delay(1000);
      
      if (await this.hasAntiBotProtection()) {
        console.warn('⚠️ Playwright Amazon DE: Anti-bot protection detected');
        return [];
      }
      ...
      for (const selector of selectors) {
        ...
        if (found) {
          ...
          console.log(`✅ Amazon DE: Found ${productElements.length} products`);
          break;
        }
      }

      if (productElements.length === 0) {
        console.log(`❌ Amazon DE: No products found`);
        return [];
      }
      ...
      console.log(`✅ Playwright Amazon DE: Extracted ${products.length} products`);
      return products;

    } catch (error) {
      console.error(`❌ Amazon DE search failed:`, error);
      return [];
    }
  }
