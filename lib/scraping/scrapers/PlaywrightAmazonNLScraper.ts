...
export class PlaywrightAmazonNLScraper extends PlaywrightBaseScraper {
  ...
  async searchProducts(searchTerm: string): Promise<ScrapedProductData[]> {
    try {
      ...
      console.log(`🔍 Playwright Amazon NL: Searching for "${searchTerm}"`);
      
      await this.navigateToUrl(searchUrl);
      console.log(`✅ Playwright Amazon NL: Navigation completed`);
      ...
      if (await this.hasAntiBotProtection()) {
        console.warn('⚠️ Playwright Amazon NL: Anti-bot protection detected');
        return [];
      }
      ...
      for (const selector of selectors) {
        ...
        if (found) {
          ...
          console.log(`✅ Amazon NL: Found ${productElements.length} products with selector: ${selector}`);
          break;
        }
      }

      if (productElements.length === 0) {
        console.log(`❌ Amazon NL: No products found`);
        return [];
      }

      const products: ScrapedProductData[] = [];
      const limit = Math.min(productElements.length, 8); // Reduced for speed
      ...
      for (let i = 0; i < limit; i++) {
        try {
          ...
          if (linkEl) {
            ...
            if (href) {
              url = href.startsWith('http') ? href : `https://www.amazon.nl${href}`;
            }
          }
          ...
          if (title && priceText) {
            ...
            console.log(`   ✅ Amazon NL: ${product.title} - €${product.price}`);
            products.push(product);
          }
        } catch (error) {
          console.log(`   ❌ Amazon NL: Error extracting product ${i + 1}:`, error.message);
          continue;
        }
      }

      console.log(`✅ Playwright Amazon NL: Extracted ${products.length} products`);
      return products;

    } catch (error) {
      console.error(`❌ Amazon NL search failed:`, error);
      return [];
    }
  }
